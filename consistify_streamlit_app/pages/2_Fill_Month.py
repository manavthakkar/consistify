...
st.set_page_config(page_title="Monthly Habit Overview", page_icon="📅")
...
def fill_month_template(month, year, habit_name, total_days, habit_array, selected_month, selected_year):
    ...
    if success_rate < 50:
        st.toast(
            f"Don't be discouraged! Your success rate is {success_rate}%. Remember, small steps lead to big changes. 💪",
            icon="💡"
        )
    elif success_rate > 80:
        st.balloons()
        st.toast(
            f"Congratulations! 🥳 You achieved a success rate of {success_rate}% for {habit_name} in {selected_month} {selected_year}!",
            icon="🎉"
        )
...
#     cookie_name='abcd',
#     cookie_key='abcd',
#     redirect_uri='http://localhost:8501/?page=fill_month',
# )
...
def fill_month_main():
    ...
    # Proceed with the page logic if logged in
    st.title("Fill Month")
        ...
        if st.button("Generate Visualization"):
            try:
                ...
                filled_image = fill_month_template(
                    month_number, int(selected_year), selected_habit, total_days, habit_array, selected_month, selected_year
                )
